...
const MemberProfile = () => {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setStatusMessage({ text: 'Password reset email sent! Check your inbox.', type: 'success' });
      setShowResetForm(false);
    } catch (error) {
      setStatusMessage({ text: 'Failed to send reset email. Please try again.', type: 'error' });
    }
  };
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      setStatusMessage({ text: 'Failed to update profile. Please try again.', type: 'error' });
    }
  };
          ...
          <div className="relative p-6 border-b border-blue-500/30">
            <div className="flex flex-col items-center">
                ...
                <div className="relative w-32 h-32 rounded-full overflow-hidden border-4 border-blue-500/50">
                  {currentUser.profilePicture?.url ? (
                    ...
                    <div className="bg-blue-900/50 w-full h-full flex items-center justify-center">
                      <span className="text-4xl">{currentUser.fullname?.charAt(0) || 'U'}</span>
                    </div>
                  )}
              ...
              <div className="mt-4 text-center">
                ...
                <p className="text-blue-300">{currentUser.designation} • {currentUser.department}</p>
                <p className="text-blue-400 text-sm mt-1">LPU ID: {currentUser.LpuId}</p>
              </div>
            </div>
          </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="bg-blue-900/30 p-4 rounded-xl">
                    ...
                    <p>{new Date(currentUser.joinedAt).toLocaleDateString() || 'Unknown'}</p>
                  </div>
                </div>
                        ...
                        {currentUser.restriction?.isRestricted && (
                          <div className="mt-4">
                            <div className="flex justify-between mb-2">
                              ...
                              <span>{new Date(currentUser.restriction.time).toLocaleString()}</span>
                            </div>
                            <div>
                              <span>Reason:</span>
                              <p className="mt-1 bg-blue-900/50 p-2 rounded">{currentUser.restriction.reason}</p>
                            </div>
                          </div>
                        )}
                      </div>

...
const AdminAuthPage = () => {
    ...
    const handleAdminLogin = async (e) => {
        ...
        setAdminError("");
        try {
            const response = await publicClient.post('/api/admin/login', adminData);
    ...
    const handleMemberRegister = async (e) => {
        ...
        setMemberSuccess("");
        try {
            const response = await publicClient.post('/api/member/register', memberData);
            if (response.status === 201 || response.status === 200) {
                setMemberSuccess("Registration successful! You can now login.");
                setMemberData({ fullName: "", LpuId: "", email: "", password: "" });
            } else {
                setMemberError(response.data?.message || "Registration failed.");
            }
        } catch (err) {
            setMemberError(err?.response?.data?.message || "Registration failed.");
        }
        setLoading(false);
    };

	...
	>
		...
		<div className="mb-2 relative">
			<label htmlFor="domains" className="block mb-2 font-medium text-white/90">
				Interested Domains <span className="text-xs text-white/60">(Select up to 2)</span>
			</label>
        ...
        ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
						>
									...
									setRegisterData((prev) => {
										...
										if (checked) {
											if (prev.domains.length < 2) {
												newDomains = [...prev.domains, domain.value];
											} else {
												newDomains = prev.domains;
											}
										} else {
											newDomains = prev.domains.filter(
												(d) => d !== domain.value
											);
										}
										return {
							...
							/>
							<span className="text-white/90">{domain.label}</span>
						</label>
					);
				})}
			</div>
			<p className="mt-1 text-xs text-white/60 pl-8">You can select up to two domains</p>
		</div>

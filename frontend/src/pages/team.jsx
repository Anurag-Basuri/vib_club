...
const LeadershipCard = ({ leader, index, onClick }) => (
    ...
    >
        <div className={`relative p-6 rounded-2xl bg-gradient-to-br from-[#0a0f1f]/80 to-[#1a1f3a]/90 border border-[#3a56c9]/40 shadow-xl cursor-pointer ${leader.level === 0 ? 'w-80' : 'w-72'}`}>
            <div className="flex flex-col items-center">
                ...
                <p className="text-[#9ca3d4] text-sm text-center mb-4">{leader.bio || 'Passionate leader driving innovation'}</p>
                <motion.button
        ...
        <div className="min-h-screen bg-gradient-to-br from-[#0a0f1f] via-[#1a1f3a] to-[#0d1326] text-white overflow-x-hidden">
            ...
            <section className="relative w-full pt-28 pb-16 px-4 flex flex-col items-center justify-center text-center">
                ...
                <div className="relative z-10 max-w-4xl mx-auto">
                    ...
                    >
                        <Sparkles className="inline-block text-[#5d7df5] animate-pulse mr-2" size={24} />
                        <span>
                            Where <span className="font-semibold text-[#5d7df5]">creativity</span> meets{' '}
                            <span className="font-semibold text-[#3a56c9]">technology</span> and{' '}
                            <span className="font-semibold text-[#0ea5e9]">collaboration</span> sparks innovation.
                        </span>
                    </motion.p>
                </div>
            </section>
            ...
            <section id="departments" className="py-16 px-4 relative z-10">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="space-y-20">
                        {technical.length > 0 && (
                            <DepartmentSection department="Technical" members={technical} onClick={handleMemberClick} />
                        )}
                        {management.length > 0 && (
                            <DepartmentSection department="Management" members={management} onClick={handleMemberClick} />
                        )}
                        {marketing.length > 0 && (
                            <DepartmentSection department="Marketing" members={marketing} onClick={handleMemberClick} />
                        )}
                        {socialMedia.length > 0 && (
                            <DepartmentSection department="Social Media" members={socialMedia} onClick={handleMemberClick} />
                        )}
                        {media.length > 0 && (
                            <DepartmentSection department="Media" members={media} onClick={handleMemberClick} />
                        )}
                        {contentWriting.length > 0 && (
                            <DepartmentSection department="Content Writing" members={contentWriting} onClick={handleMemberClick} />
                        )}
                        {design.length > 0 && (
                            <DepartmentSection department="Design" members={design} onClick={handleMemberClick} />
                        )}
                        {hr.length > 0 && (
                            <DepartmentSection department="HR" members={hr} onClick={handleMemberClick} />
                        )}
                        {eventManagement.length > 0 && (
                            <DepartmentSection department="Event Management" members={eventManagement} onClick={handleMemberClick} />
                        )}
                    </div>
                </div>
            </section>

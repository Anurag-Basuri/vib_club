...
import { 
  Search, Filter, UserPlus, Users, User, Info, Pencil, Ban, Trash2, RotateCcw, 
  Eye, EyeOff, ChevronDown, XCircle, CheckCircle, AlertCircle, Loader2
} from 'lucide-react';
import {
  useGetAllMembers,
  useBanMember,
...
const MemberRow = React.memo(({ 
  ...
  <tr key={member._id} className="hover:bg-gray-750/50 transition">
    ...
    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">{member.designation || '-'}</td>
  ...
  return (
    <Modal title="Register New Member" onClose={onClose}>
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          <label className="block text-gray-300 mb-1">Designation <span className="text-red-400">*</span></label>
          <select
            className="w-full px-3 py-2 bg-gray-700/50 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <tbody className="bg-gray-800 divide-y divide-gray-700">
              {filteredMembers.map((member) => (
                <MemberRow 
                  ...
                  actionLoading={banLoading || unbanLoading || removeLoading || updateLoading}
                />
              ))}
            </tbody>
          </table>

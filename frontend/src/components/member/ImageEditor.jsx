...
import {
    RotateCw, ZoomIn, ZoomOut, RefreshCw, X,
    Upload, Save, Edit3, FlipHorizontal, FlipVertical,
    Contrast, Palette, Crop, RotateCcw, Grid, Eye, EyeOff
} from 'lucide-react';

const CONTROL_BTN = 'inline-flex items-center justify-center w-9 h-9 rounded-lg bg-gray-100 dark:bg-gray-800 hover:bg-blue-100 dark:hover:bg-blue-700 transition-colors text-gray-700 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-400';
...
const TAB_BTN_INACTIVE = 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200 hover:bg-blue-50 dark:hover:bg-blue-700';
...
const ImageEditor = ({
    ...
    useEffect(() => {
        const updateSize = () => {
            if (containerRef.current) {
                ...
                const maxSize = Math.min(container.offsetWidth - 16, window.innerHeight * 0.45, 500);
    ...
    }, [transform, imageLoaded, canvasSize, brightness, contrast, saturation, filterIndex, viewOriginal, aspectRatio, showGrid]);
    ...
    const handleSave = async () => {
        ...
        outputCtx.scale(scale * (flipX ? -1 : 1) * scaleFactor, scale * (flipY ? -1 : 1) * scaleFactor);
        outputCtx.drawImage(img, -img.width / 2 + (x * scaleFactor), -img.height / 2 + (y * scaleFactor));
        outputCtx.restore();
        ...
        <div className="space-y-4 sm:space-y-6">
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Aspect Ratio</span>
                </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Zoom</span>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <button
                        className={CONTROL_BTN}
                        onClick={() => updateTransform({ scale: Math.min(5, transform.scale + 0.1) })}
                    >
                        <ZoomIn size={18} />
                    </button>
                </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Rotate</span>
                    <span className="text-xs text-gray-500">{transform.rotation}°</span>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        className={CONTROL_BTN}
                        onClick={() => updateTransform({ rotation: (transform.rotation - 90 + 360) % 360 })}
                    >
                        <RotateCcw size={18} />
                    </button>
                    <input
    ...
    const renderAdjustControls = () => (
        <div className="space-y-4 sm:space-y-6">
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Brightness</span>
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Contrast</span>
            ...
            <div>
                <div className="flex items-center justify-between mb-1">
                    <span className="font-medium text-gray-700 dark:text-gray-200 text-sm sm:text-base">Saturation</span>
                    ...
                    <div className="flex-1 flex flex-col lg:flex-row gap-2 sm:gap-4 p-1 sm:p-4 overflow-y-auto">
                        ...
                        {isEditing && (
                            <div className="w-full lg:w-80 space-y-4 lg:space-y-6 overflow-auto pt-2 lg:pt-0">
                                ...
                                {/* Controls Content */}
                                <div className={`${controlsExpanded ? 'block' : 'hidden lg:block'} max-h-[40vh] lg:max-h-none overflow-y-auto`}>
                                    {/* Tabs */}
                                    <div className="flex gap-1 sm:gap-2 mb-2 overflow-x-auto pb-1">
                                        <button
                                            className={activeTab === 'transform' ? TAB_BTN + ' ' + TAB_BTN_ACTIVE : TAB_BTN + ' ' + TAB_BTN_INACTIVE}
                                            onClick={() => setActiveTab('transform')}
                                        >
                                            Transform
                                        </button>
                                        <button
                                            className={activeTab === 'adjust' ? TAB_BTN + ' ' + TAB_BTN_ACTIVE : TAB_BTN + ' ' + TAB_BTN_INACTIVE}
                                            onClick={() => setActiveTab('adjust')}
                                        >
                                            Adjust
                                        </button>
                                        <button
                                            className={activeTab === 'filters' ? TAB_BTN + ' ' + TAB_BTN_ACTIVE : TAB_BTN + ' ' + TAB_BTN_INACTIVE}

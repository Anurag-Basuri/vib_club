				...
				<div className="flex items-center mb-4">
					<div className="relative">
						...
						<img
							...
							alt={member?.fullName}
							className="w-14 h-14 sm:w-16 sm:h-16 rounded-full object-cover border-2 border-[#3a56c9]/30 relative z-10"
							onError={(e) => {
								e.target.src = '/default-profile.png';
							}}
						/>
						<motion.div
							className="absolute -top-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-gradient-to-r from-[#5d7df5] to-[#3a56c9] flex items-center justify-center shadow-lg"
							initial={{ rotate: 0 }}
							animate={{ rotate: 360 }}
					...
					<div className="ml-3">
						<h4 className="text-base sm:text-lg font-semibold text-white group-hover:text-[#5d7df5] transition-colors line-clamp-1">
							{member.fullName}
						</h4>
						<p className="text-[#5d7df5] text-xs sm:text-sm">{member.designation}</p>
					</div>
				</div>
